...
line, a commit closing the PR's own issue silent, one series read per merge (a second only when the
gate settles on another head), and a capped, short or unread series saying the scan did not run.
