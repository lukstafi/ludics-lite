...
bounds slots x width at 8, its device-wide SDMA queue pool ([linux-boxes.md](linux-boxes.md)).
tuf-amd-linux keeps one slot until it is measured. The kind does not silently change the
