...
worktree and an independent conversation. Do not use `codex exec`, tmux or `fleet-worker.sh
launch --kind codex` for these explicitly selected app workers.

Before any execution leg, follow [execution reservations](executions.md), including
coordinator integration. Agent host selection does not reserve machine execution capacity.
...
processes can still write, and no outstanding execution reservation uses the checkout, before cleanup. Keep the thread and its app-managed worktree through
