...
Close the report with the routine-drift finding from step 0 — below step 1's "updated locally but not pushed" line, if that one applies, so neither hides the other: the verdict line for each local routine, the branch and the `rev-list` counts whenever they are not `main` and `0 0`, or the fetch's error if the comparison could not be made, and — when anything drifted — the diff `sync-routines.sh` printed and the repair a PERSON runs, in the direction the diff shows. `push` ONLY when the checkout is canonical — on `main`, with `0 0` from the counts above; that is the usual case, a prompt edit that merged here and was never installed. Anything else, recommend no push and say what the checkout is: a nonzero BEHIND wants `git -C ~/ludics-lite checkout main && git -C ~/ludics-lite merge --ff-only origin/main` first, by name (not a bare `git pull --ff-only`, which follows whatever the CURRENT branch tracks — on a checkout parked on a topic branch it fast-forwards that topic and installs a prompt still older than `main`), and a nonzero AHEAD, or any branch but `main`, wants the prompt edit landed through review before anything is installed. Never recommend `push` out of an ahead checkout: that publishes unmerged text to the live scheduler, which is a worse failure than the drift it would be fixing. `pull` when the newer text is the INSTALLED copy: a routine that edited its own prompt in place is a supported workflow (`routines/README.md`), and a `push` over it destroys the only copy of that edit before anyone has committed it. Never recommend a direction the diff does not show; when it shows both sides having moved, say so and recommend neither. Say it even when everything is in sync, in one line: a check whose silence and whose absence look alike is not a check. Re-run the script first if anything this run did could have moved either side.
