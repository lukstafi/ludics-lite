...
- Pick the issues that can be implemented or solved first.
  ...
  - Among them, show which ones prefer to run on ROG (NVIDIA GPU), which on Minix (AMD GPU, unified memory), and which on TUF (AMD GPU, discrete memory).
- Place by memory model on the AMD side. `minix-amd-linux` (Radeon 8060S, `gfx1151`) shares system memory with its CPU; `tuf-amd-linux` (Radeon RX 7700S, `gfx1102`, discrete RDNA3 with its own VRAM) is the box that exposes what unified memory masks. HIP work whose behaviour depends on discrete versus unified memory — host↔device transfers, buffer placement, pinned or zero-copy host memory, anything minix's shared memory could hide — homes on TUF, or takes a TUF leg beside minix's; HIP work where the memory model does not matter stays on minix. Give the Machine-placement section a TUF paragraph for these legs, as it has one per other box. Every HIP measurement the plan cites or asks for says which memory model it ran on (unified on minix, discrete on TUF), since a number from one does not stand in for the other. TUF is on Wi-Fi only and a person wakes it by hand, so write "manual wake" beside each TUF placement: the coordinator then asks the user to wake it or defers the item with the gate recorded, and never reads "asleep" as a failure.
