...
#
# A fifth compares ship-pr/SKILL.md with post-merge-cleanup.sh's usage text, which the helper
# prints from the one option table its parser also reads (ludics-lite#332): every option the text
# lists is named in the prompt, and every `--option` the prompt's fenced command lines pass to the
# helper is one the text lists -- the two files ludics-lite#276 found edited apart (see
# `check_cleanup_options` for what "of the helper" means on the prompt's side).
#
...
# this pins the one fact a lookup can see, in both directions: every option usage()'s text
...
# The arity the listing gives each option -- whether a `<value>` placeholder follows it -- is
# what the prompt's side skips a value by. It is not compared with the helper's parser, because
# there is no second copy to compare: the helper declares each option once, in its option table,
# and both usage() and the parse loop are driven by that table (ludics-lite#332, which retired the
# parser reader that ludics-lite#302's `--force-integrated` drift had called for). So the listing
# is read where the table is rendered -- the helper's printed usage text -- and not off its source.
CLEANUP_HELPER=ship-pr/scripts/post-merge-cleanup.sh
CLEANUP_PROMPT=ship-pr/SKILL.md

# usage_options <script>: the options the helper's usage text lists, one per line as
# `--name<TAB>arity`. The helper is RUN, with no arguments, which is its usage error: it prints the
# text on stderr and exits 2 without touching any repository (it has no checkout arguments to
# touch). An exit other than 2, or a stderr not opening with `usage: `, is not a usage text and
# fails the function, since reading a crash's output as an empty listing would hold the prompt to
# nothing. Of the text, each line opening with two blanks and a `--name` is a listing line, the
# shape usage() prints a table row in (`  --base <branch>  Base branch …`), and the arity is 1 when
# a `<value>` placeholder follows the name, else 0. The prose below the listing mentions an option
# mid-sentence, at no such indent, and is not read; nor is the source, whose table is the helper's
# business.
usage_options() {
  local text rc=0
  text=$(bash "$1" </dev/null 2>&1 >/dev/null) || rc=$?
  [ "$rc" -eq 2 ] || return 1
  case "$text" in 'usage: '*) ;; *) return 1 ;; esac
  awk '
    match($0, /^  --[A-Za-z0-9][A-Za-z0-9-]*([[:space:]]|$)/) {
      o = substr($0, 3, RLENGTH - 2); sub(/[[:space:]]$/, "", o)
      print o "\t" (substr($0, RLENGTH + 1) ~ /^[[:space:]]*</ ? 1 : 0)
    }
  ' <<<"$text"
}
...
check_cleanup_options() {
  ...
  if ! listed=$(usage_options "$ROOT/$CLEANUP_HELPER"); then
    ko "$CLEANUP_HELPER" "run with no arguments, it printed no usage text: it must exit 2 with a stderr opening 'usage: ' (see usage_options)"
    return 0
  fi
  if [ -z "$listed" ]; then
    # Not a pass: an empty register would hold the prompt to nothing, in silence.
    ko "$CLEANUP_HELPER" "usage() lists no options this reader can see: a usage line opening with two blanks and a --name"
    return 0
  fi
