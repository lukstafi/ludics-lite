...
a comment's `Reviewed commit:` stamp — and `watch` compares that to the head it reads after each
poll. Reviewer activity about some other commit is a *previous* round scrolling past above the
...
Every one of those verdicts polls once more before it prints, and a round found by that last look
wins: the seconds a state read takes are exactly when a round lands, and a nudge posted over one
re-requests the review and CLEARS the 👍 it was about to get. The grace itself is measured from
